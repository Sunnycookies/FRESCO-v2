...
class AttentionControl():
    ...
    * collect intermediate attention feature (for spatial-guided attention)
    ...
    def set_qk_injection_timesteps(self, qk_injection_timesteps):
        self.qk_injection_timesteps = qk_injection_timesteps
    ...
    def __call__(
        ...
        if encoder_hidden_states is None:
            ...
            if self.controller and self.controller.store:
                # print(f"diff: controller stores shape {hidden_states.shape}")
            ...
            if self.controller.attn_mask is not None:
                for m in self.controller.attn_mask:
                    if m.shape[1] == key.shape[1]:
                        attn_mask = m # efficient cross-frame attention
                        #attn_mask = (m * 0.0 + 1.0).bool() # original cross-frame attention
            # BC * HW * 8D --> B * C * HW * 8D
        ...
        if self.controller and self.controller.use_interattn and (not crossattn):
            # print('sdedit temporal attention')
            del query, key, value
            torch.cuda.empty_cache()
            bwd_mapping = None
            fwd_mapping = None
    ...
    while n_iter[0] < iters:
        def closure():
            ...
            if optimize_temporal and flows is not None and occs is not None:
                ...
                # print(f"flow_warp: c1({c1.shape}) bwd_flow_({bwd_flow_.shape})")
                # print(f"flow_warp: c2({c2.shape}) fwd_flow_({fwd_flow_.shape})")
    ...
    def forward(
        ...
        for downsample_block in self.down_blocks:
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                ...
                # print(f"\ndiffusion_hacked: sample shape: {sample.shape} encoder_hidden_states: {encoder_hidden_states.shape}\n")
